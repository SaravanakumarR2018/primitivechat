...
@app.post("/tickets", response_model=TicketResponse, status_code=status.HTTP_201_CREATED, tags=["Ticket Management"])
    ...
    try:
        ...
        if db_response["status"] == "created":
            ...
            raise HTTPException(
                ...
                detail=db_response.get("message", "Failed to create ticket")
            )

    except ValueError as e:
        # Custom field validation error
        logger.error(f"Custom field error: {e}")
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST,
            detail=str(e)
        )
        ...
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST,
            detail="Failed to create ticket due to database issue"
        )
    ...
    try:
        ...
        if update_status["status"] == "updated":
            ...
            raise HTTPException(
                ...
                detail=f"Ticket ID {ticket_id} not found for customer {customer_guid}"
            )
        elif update_status["status"] == "conflict":
            raise HTTPException(
                status_code=HTTPStatus.CONFLICT,
                detail=f"Update conflict: {update_status['reason']}"
            )
        elif update_status["status"] == "bad_request":
            raise HTTPException(
                status_code=HTTPStatus.BAD_REQUEST,
                detail=f"Invalid request: {update_status['reason']}"
            )
        else:
            raise HTTPException(
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
                detail=f"Unexpected error during ticket update: {update_status['reason']}"
            )
    ...
    try:
        ...
        if result["status"] == "deleted":
            ...
            raise HTTPException(
                ...
                detail=f"Ticket ID {ticket_id} does not exist. No Action Needed"
            )
