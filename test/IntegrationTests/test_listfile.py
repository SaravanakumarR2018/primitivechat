...
class TestListFileAPI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertIsNotNone(self.customer_guid, "Customer GUID is missing in the response")
        logger.info(f"Received valid customer_guid: {self.customer_guid}")

        logger.info("=== Setup process completed ===")

    def test_list_files_from_valid_customer(self):
        ...
        #Upload files for the new customer
        ...
        for filename,content,mime_type in files_to_upload:
            ...
            if upload_response.status_code==200:
                logger.info(f"Successfully uploaded file: {filename}. Status code 200 received.")
            else:
                logger.error(f"File upload failed for {filename}. Status code: {upload_response.status_code}. Expected: 200.")
                raise AssertionError(f"Expected status code 200 but got {upload_response.status_code} for file upload")

            logger.info(f"Upload Response for {filename}: {upload_response.json()}")
            self.assertEqual(upload_response.status_code, 200, f"File upload failed for {filename}")

        #List files in the new customer's bucket
        ...
        self.assertEqual(set(listed_files), set(expected_file_names),
                         "File names in response do not match expected files")
